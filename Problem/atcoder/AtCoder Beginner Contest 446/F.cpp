...
class Graph
{
    std::vector<std::vector<std::pair<u32, T>>> adj;
public:
    Graph(u32 n): adj(n + 1) {}
    ...
    [[nodiscard]] std::vector<std::pair<u32, T>>& operator[](i32 u)
    {
        return adj[u];
    }
};
...
void solve()
{
    ...
    pq<i32, std::greater<>> heap;
    heap.push(1);

    vb visited(n + 1), in_heap(n + 1);
    i32 vcnt = 0;
    for (i32 i = 1; i <= n; i++)
    {
        ...
        std::cout << (vcnt == i? i32(heap.size()): -1) << "\n";
    }
}
