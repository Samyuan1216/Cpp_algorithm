...
void solve()
{
    ...
    auto a = std::vector<mint>(n);
    for (int i = 1; i < n; ++i)
    {
        std::cin >> a[i];
    }

    auto b = std::vector<mint>(m);
    for (int i = 1; i < m; ++i)
    {
        std::cin >> b[i];
    }

    auto sum1 = std::vector<mint>(n + 1), sum2 = std::vector<mint>(n + 1);
    for (int i = 0; i < n; ++i)
    {
        sum1[i + 1] = sum1[i] + a[i];
    ...
    for (int j = 1; j < m; ++j)
    {
        ...
        for (int k = 0; j * k < n; ++k)
        {
            int l = j * k, r = std::min(j * (k + 1), n);
            res += (sum2[r] - sum2[l]) - (sum1[r] - sum1[l]) * (j * k);
        }
