...
void solve()
{
    ...
    for (i32 i = 0; i < n; i++)
    {
        if (pre[i] == arr[i] - 1)
        {
            dp[i] = (sum[indexs[arr[i]][0]] - sum[0] + mod + 1) % mod;
        }
        else if (pre[i] > arr[i] - 1)
        {
            dp[i] = (sum[indexs[arr[i]][pre[i] - arr[i] + 1]] - sum[indexs[arr[i]][pre[i] - arr[i]] + 1] + mod) % mod;
        }
