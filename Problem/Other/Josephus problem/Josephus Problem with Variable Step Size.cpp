...
void solve()
{
    i32 n = 100, v = 500, test = 10000;
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_real_distribution dis(0.0, 1.0);

    std::cout << "测试开始\n";
    for ([[maybe_unused]] i32 i: views::iota(0, test))
    {
        i32 size = dis(gen) * n + 1;
        auto random_array = [&]()
        {
            vi32 arr(size);
            for (i32 i: views::iota(1, size))
            {
                arr[i] = dis(gen) * v + 1;
            }
        ...
        auto joseph1 = [&]()
        {
            ...
            for (i32 i: views::iota(1, size))
            {
                for ([[maybe_unused]] i32 cnt: views::iota(1, arr[i]))
                {
                    pre = cur;
                    cur = next[cur];
                }
        ...
        auto joseph2 = [&]()
        {
            ...
            i32 ans = 1;
            for (i32 c = 2, i = size - 1; c <= size; c++, i--)
            {
                ans = (ans + arr[i] - 1) % c + 1;
            }
