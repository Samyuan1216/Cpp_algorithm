...
void solve()
{
    ...
    Seg_Tree<Info, Tag> tr(n, n + 1);
    std::vector<int> pos(MAXN, -1);

    for (int i = n - 1; i >= 0; --i)
    {
        int pre = (i == n - 1? 0: i + 2), cur = i + 1;
        if (pos[arr[i]] != -1)
        {
            tr.modify(pos[arr[i]], pos[arr[i]], cur, pre, {-1});
            tr.modify(i, i, cur, cur, {1});
        }
        else
        {
            tr.modify(i, i, cur, pre, {1});
        }
    ...
    while (m--)
    {
        ...
        std::tie(l, r) = std::tuple((l + last) % n + 1, (r + last) % n + 1);
        std::tie(l, r) = std::tuple(std::min(l, r) - 1, std::max(l, r) - 1);

        int sum = tr.query(l, r, l + 1).sum;
        int k = (sum + 1) / 2;

        int res = *tr.find_kth(k, l + 1, 0, [](const Info &info)
        {
            return i64(info.sum);
        });
