...
void solve()
{
    ...
    struct Info
    {
        ...
        Info operator+(const Info &other) const
        {
            ...
            res.k = std::max(k, other.k);

            if (max > other.min)
            {
                res.k = std::max(res.k, len);
            }
    ...
    int power = std::bit_ceil(unsigned(n));
    std::vector<Info> init(power);
