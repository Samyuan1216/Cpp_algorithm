...
void solve()
{
    ...
    for (i32 i: views::iota(0, n))
    {
        ...
        {
            a.push_back(vi32{x, i});
        }
    }

    for (i32 i: views::iota(1, i32(a.size())))
    {
        a[i - 1][1] = a[i][1] - a[i - 1][1];
    }
    a.back()[1] = n - a.back()[1];

    v2i32 dp(k + 1, vi32(k + 1, -1));
    dp[0][0] = 0;

    for (auto &p: a)
    {
        v2i32 f(k + 1, vi32(k + 1, -1));
        for (i32 s: views::iota(0, k + 1))
        {
            i32 max = -1;
            for (i32 m: views::iota(0, k + 1))
            {
                max = std::max(max, dp[s][m]);
                f[s][m] = max;
            }
        }

        v2i32 next(k + 1, vi32(k + 1, -1));
        for (i32 s: views::iota(0, k + 1))
        {
            for (i32 m: views::iota(0, k + 1))
            {
                if (dp[s][m] != -1)
                {
                    next[s][m] = std::max(next[s][m], dp[s][m] + p[1] * m);
                }
            }

            for (i32 m: views::iota(1, std::min(p[0], s) + 1))
            {
                if (f[s - m][m] != -1)
                {
                    next[s][m] = std::max(next[s][m], f[s - m][m] + p[1] * m);
                }
            }
        }
