...
void solve()
{
    ...
    auto m = std::array<mint, 3>();
    for (int i = 0; i < 3; ++i)
    {
        m[i] = (r + (3 - i)) / 3 - (l - 1 + (3 - i)) / 3;
    }

    auto dp = std::vector(n, std::array<mint, 3>());
    dp[0] = m;

    for (int i = 1; i < n; ++i)
    {
        dp[i][0] = dp[i - 1][0] * m[0] + dp[i - 1][1] * m[2] + dp[i - 1][2] * m[1];
        dp[i][1] = dp[i - 1][0] * m[1] + dp[i - 1][1] * m[0] + dp[i - 1][2] * m[2];
        dp[i][2] = dp[i - 1][0] * m[2] + dp[i - 1][1] * m[1] + dp[i - 1][2] * m[0];
    }
