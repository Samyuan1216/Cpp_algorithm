...
template<typename T>
pair<std::vector<T>, std::vector<T>> fact_inv(i32 n, T mod)
{
    std::vector<T> fac(n + 1), inv(n + 1);
...
void solve()
{
    ...
    for (i32 i = 0; i <= n; i++)
    {
        g[i] = (g[i] + mod - 1) * C(fac[n], inv[i], inv[n - i], mod) % mod;
    }
    ...
    for (i32 i = k; i <= n; i++)
    {
        if ((i - k) & 1)
        {
            ans = (ans + C(fac[i], inv[k], inv[i - k], mod) * g[i] % mod * (mod - 1) % mod) % mod;
        }
        else
        {
            ans = (ans + C(fac[i], inv[k], inv[i - k], mod) * g[i] % mod) % mod;
        }
    }
