    ...
    while (m--)
    {
        ...
        if (op == 'P')
        {
            ...
            while (top[u] != top[v])
            {
                ...
                {
                    tr.modify(dfn[top[u]], dfn[u], {1});
                    u = father[top[u]];
                }
                else
                {
                    tr.modify(dfn[top[v]], dfn[v], {1});
                    v = father[top[v]];
                }
            }

            tr.modify(std::min(dfn[u], dfn[v]) + 1, std::max(dfn[u], dfn[v]), {1});
        }
        else
        {
            int u, v;
            ...
            int down = std::max(dfn[u], dfn[v]);
            std::cout << tr.query(down, down).sum << "\n";
        }
    }
}
