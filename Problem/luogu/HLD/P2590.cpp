...
void solve()
{
    ...
    struct Info
    {
        ...
        {
            return {sum + other.sum, std::max(max, other.max), len + other.len};
        }
    ...
    for (int i = 0; i < n; ++i)
    {
        init[i].max = init[i].sum = arr[seg[i]];
    }
    ...
    while (q--)
    {
        ...
        else if (op[1] == 'M')
        {
            ...
            ans = std::max(ans, tr.query(std::min(dfn[u], dfn[v]), std::max(dfn[u], dfn[v])).max);
            std::cout << ans << "\n";
        }
        else
        {
            ...
            ans += tr.query(std::min(dfn[u], dfn[v]), std::max(dfn[u], dfn[v])).sum;
            std::cout << ans << "\n";
        }
    }
}
