...
void solve()
{
    ...
    while (op[0] != 'S')
    {
        if (op[1] == 'h')
        {
            ...
            tr.modify(dfn[edge[k]], dfn[edge[k]], {0, w, true});
        }
        else if (op[1] == 'o')
        {
            int u, v, w;
            ...
            while (top[u] != top[v])
            {
                ...
                {
                    tr.modify(dfn[top[u]], dfn[u], {0, w, true});
                    u = father[top[u]];
                }
                else
                {
                    tr.modify(dfn[top[v]], dfn[v], {0, w, true});
                    v = father[top[v]];
                }
            }

            tr.modify(std::min(dfn[u], dfn[v]) + 1, std::max(dfn[u], dfn[v]), {0, w, true});
        }
        else if (op[0] == 'A')
        {
            int u, v, w;
            ...
            while (top[u] != top[v])
            {
                if (deep[top[u]] > deep[top[v]])
                {
                    tr.modify(dfn[top[u]], dfn[u], {w, 0, false});
                    u = father[top[u]];
                }
                else
                {
                    tr.modify(dfn[top[v]], dfn[v], {w, 0, false});
                    v = father[top[v]];
                }
            }

            tr.modify(std::min(dfn[u], dfn[v]) + 1, std::max(dfn[u], dfn[v]), {w, 0, false});
        }
        else
        {
            int u, v;
