...
void solve()
{
    ...
    struct Info
    {
        i64 max = -1e18, min = 1e18, lprofit = 0, rprofit = 0;

        Info operator+(const Info &other)
        {
            return {std::max(max, other.max), std::min(min, other.min),
                    std::max({lprofit, other.lprofit, other.max - min}), std::max({rprofit, other.rprofit, max - other.min})};
        }
        ...
        Info reversed()
        {
            return {max, min, rprofit, lprofit};
        }
    };
    ...
    for (int i = 0; i < n; ++i)
    {
        init[i].max = init[i].min = arr[seg[i]];
    }
    ...
    while (q--)
    {
        ...
        while (top[a] != top[b])
        {
            ...
            {
                apath = apath + tr.query(dfn[top[a]], dfn[a]).reversed();
                a = father[top[a]];
            }
            else
            {
                bpath = tr.query(dfn[top[b]], dfn[b]) + bpath;
                b = father[top[b]];
            }
        }
        ...
        if (deep[a] > deep[b])
        {
            apath = apath + tr.query(dfn[b], dfn[a]).reversed();
        }
        else
        {
            bpath = tr.query(dfn[a], dfn[b]) + bpath;
        }

        std::cout << std::max({apath.lprofit, bpath.lprofit, bpath.max - apath.min}) << "\n";
        ...
        while (top[a] != top[b])
        {
            if (deep[top[a]] > deep[top[b]])
            {
                tr.modify(dfn[top[a]], dfn[a], {v});
                a = father[top[a]];
            }
            else
            {
                tr.modify(dfn[top[b]], dfn[b], {v});
                b = father[top[b]];
            }
        }

        tr.modify(std::min(dfn[a], dfn[b]), std::max(dfn[a], dfn[b]), {v});
    }
}
