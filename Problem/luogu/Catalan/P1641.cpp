...
template<typename T>
std::vector<T> get_fact(i32 n, T mod = lim<T>::max())
{
...
template<typename T>
std::vector<T> get_inv(std::vector<T> &fac, i32 n, T mod = lim<T>::max())
{
...
template<typename T>
T C(T fac, T inv1, T inv2, T mod = lim<T>::max())
{
...
void solve()
{
    constexpr i64 mod = 20100403;
    i32 n, m;
    std::cin >> n >> m;

    auto fac = get_fact(n + m, mod);
    auto inv = get_inv(fac, n + m, mod);

    std::cout << (C(fac[n + m], inv[m], inv[n], mod) - C(fac[n + m], inv[m - 1], inv[n + 1], mod) + mod) % mod << "\n";
}
