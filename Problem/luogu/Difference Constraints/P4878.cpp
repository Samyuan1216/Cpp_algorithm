...
void solve()
{
    ...
    Graph<i32> g(n, 2 * n + m1 + m2);
    for (i32 i = 1; i <= n; i++)
    {
        g.add_edge(0, i, 0);
    }

    for (i32 i = 1, u, v, w; i <= m1; i++)
    {
        std::cin >> u >> v >> w;
    ...
    auto [status, _] = spfa(g, 0, n + 1, INT32_MAX, [](i32 u, i32 w, i32 v)
    {
        return u + w < v;
    });
    ...
    {
        auto [_, dist] = spfa(g, 1, n + 1, INT32_MAX, [](i32 u, i32 w, i32 v)
        {
            return u + w < v;
        });

        std::cout << (dist[n] == INT32_MAX? -2: dist[n]) << "\n";
    }
}
