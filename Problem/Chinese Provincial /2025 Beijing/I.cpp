...
void solve()
{
    ...
    i128 ans = -1, max = std::max(a, b);
    for (i128 l = 1, r; l <= max; l = r + 1)
    {
        i128 ca = (a + l - 1) / l, cb = (b + l - 1) / l;
        i128 ra = (ca == 1? max: (a - 1) / (ca - 1));
        i128 rb = (cb == 1? max: (b - 1) / (cb - 1));
        r = std::min({ra, rb, max});

        i128 x = l * ca - a, y = l * cb - b;
        if (x <= k && y <= k)
        {
            i128 lcm = l * ca * cb;
            if (ans == -1 || lcm < ans)
